...
function logApiError_(action, requestId, err, note) {
  console.error('[API ERROR]', {
    ...
    status: err?.meta?.status || undefined,
    message: err?.message,
    note: note || undefined,
  });
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // P1-01 — only load runtime data if API is configured;
  // otherwise skeletons persist indefinitely (no backend = no data).
  if (runtimeApiUrl_) {
    loadDashboard();
...
function wireLoginGate_() {
  ...
  function doGateLogin() {
    ...
    apiPost('login', { username: username, password: password })
      .then((res) => {
        if (res?.success !== true || !res.sessionToken) {
          const msg = res?.error?.message || 'Login gagal.';
          throw new Error(msg);
        }
...
function mapConnectionTestError_(err) {
  if (err instanceof ApiError) {
    switch (err.code) {
      ...
      case 'HTTP':
        return (
          'Tidak dapat terhubung ke Google Apps Script' + (err.meta?.status ? ` (HTTP ${err.meta.status})` : '') + '.'
        );
      case 'PARSE':
 ...
 * error (API belum diatur). Dua state ini TIDAK dicampur (spec §12).
...
function onAjukanKodeAlatChange_() {
  ...
  const ukuranSuffix = toolForUkuran?.spesifikasi ? ` (${toolForUkuran.spesifikasi})` : '';
  unitSel.innerHTML =
  ...
  form.addEventListener('submit', (e) => {
    ...
    apiPost('submitPengajuanTumpul', {
      ...
      .then((res) => {
        btn.disabled = false;
        if (res?.success !== true) {
          showAjukanTumpulMsg_('error', res?.error?.message || 'Gagal mengajukan. Coba lagi.');
          return;
        }
      ...
      .catch((err) => {
        btn.disabled = false;
        showAjukanTumpulMsg_('error', err?.message || 'Gagal mengajukan. Coba lagi.');
      });
  });
}
...
function handleKonfirmasiTumpul_(id, approve, triggerBtn) {
  ...
  apiPost('confirmPengajuanTumpul', {
    ...
    .then((res) => {
      if (res?.success !== true) {
        showKonfirmasiTumpulMsg_('error', res?.error?.message || 'Gagal memproses pengajuan.');
        row.querySelectorAll('button').forEach((b) => {
          b.disabled = false;
        });
        return;
    ...
    .catch((err) => {
      showKonfirmasiTumpulMsg_('error', err?.message || 'Gagal memproses pengajuan.');
      row.querySelectorAll('button').forEach((b) => {
        b.disabled = false;
      });
    });
...
function loadFormReferenceData() {
    ...
    .catch((err) => {
      ...
      if (hint) {
        hint.textContent =
          ...
          (err?.message ? err.message : 'error tidak diketahui') +
  ...
  apiGet('getBackendVersion')
    .then((res) => {
      console.log('[APP] Backend version:', res?.version || '(tidak diketahui)');
    })
    .catch((err) => {
      console.warn(
...
function onKodeAlatChange() {
  ...
  if (!kodeAlat) {
    setUnitMode_('select');
    unitSel.innerHTML = `<option value="">${driven ? 'Pilih mesin & kode alat dulu…' : 'Pilih kode alat dulu…'}</option>`;
    unitSel.disabled = true;
    unitManual.value = '';
    unitHint.style.display = 'none';
...
function validateTransactionResponse_(res) {
  ...
  const errMsg = res.error?.message || 'Transaksi gagal disimpan.';
...
function mapErrorToMessage_(err, isNewUnit) {
  ...
  switch (err.code) {
    ...
    case 'HTTP':
      return `Request gagal${err.meta?.status ? ` (HTTP ${err.meta.status})` : ''}. Coba lagi.`;
    default:
